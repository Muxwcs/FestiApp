  ...
  return (
    <>
        ...
        {array.slice(0, showAll ? array.length : showedItems).map((timelineItem: TimelineItem, index) => {
          return (
            <div key={index} className="relative pl-8 sm:pl-32 py-6 group">
              <div className="flex flex-col sm:flex-row items-start mb-1 group-last:before:hidden before:absolute before:left-2 sm:before:left-0 before:h-full before:px-px before:bg-slate-300 sm:before:ml-28 before:self-start before:-translate-x-1/2 before:translate-y-3 after:absolute after:left-2 sm:after:left-0 after:w-2 after:h-2 after:bg-cyan-600 after:border-4 after:box-content after:border-slate-50 after:rounded-full sm:after:ml-28 after:-translate-x-1/2 after:translate-y-1.5">
                ...
                <div className="text-lg font-bold text-white!">{timelineItem.title}</div>
              </div>
              <div className={`z-10 w-fit flex text-md font-medium dark:text-white bg-muted rounded-full px-2 py-1 mb-2 border ${timelineItem.imageSrc !== "" ? "absolute right-1 top-6 sm:top-15" : ""}`}>
              ...
              {timelineItem.imageSrc !== "" ?
                <div className="relative">
                  ...
                  />
                  <div className="absolute bottom-0 left-0 right-0 h-full bg-linear-to-t from-muted from-10% to-transparent rounded-xl"></div>
      ...
      {array.length > showedItems && (
        <button className="border bg-white text-emerald-400 cursor-pointer hover:underline p-2 rounded-full absolute right-0" onClick={toggleShowAll} style={{ zIndex: 2 }}>
