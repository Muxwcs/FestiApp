...
import { ArrowLeft, ClipboardCheck, Search, User, Mail, Phone } from "lucide-react"
...
export function ReferentSectorDetail({ sector }: { sector: SectorData }) {
  ...
  const filteredVolunteers = sector.volunteers.filter((v) => {
    ...
    const matchesTimeslot = filterTimeslot === "all" ||
      v.assignments.some((a) => a.timeslotName === filterTimeslot)
                  ...
                  {filteredVolunteers.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {filteredVolunteers.map(({ volunteer: vol, assignments }) => (
                            <tr key={vol.id} className="border-b hover:bg-muted/50">
                              ...
                              <td className="p-2">
                                <div className="flex flex-wrap gap-1">
                                  {assignments.map((a) => (
                                    <Badge key={a.affectationId} variant="outline" className="text-xs">{a.timeslotName}</Badge>
                                  ))}
                                </div>
                              </td>
                              <td className="p-2">
              ...
              {sector.timeslots.length === 0 ? (
                <Card>
                  <CardContent className="py-8">
                    <p className="text-sm text-muted-foreground text-center">Aucun créneau à pointer</p>
                  </CardContent>
                </Card>
              ) : (
                sector.timeslots.map((ts) => {
                  const tsVolunteers = sector.volunteers.flatMap((v) =>
                    v.assignments
                      .filter((a) => a.timeslotId === ts.id)
                      .map((a) => ({
                  ...
                  return (
                    <PresenceChecklist
                      ...
                      dateStart={ts.dateStart}
                      dateEnd={ts.dateEnd}
                      volunteers={tsVolunteers}
                    />
                  )
                })
              )}
